...
if not os.path.isdir('emonet-pytorch'):
    os.system(f"git clone https://github.com/cindyhfls/emonet-pytorch.git")
emonet_abs_path = os.path.abspath('emonet-pytorch')
...
def get_vision_model(args, device):
    """
    Load a pre-trained video model
    ...
    if args.model_name == 'emonet':
        model = EmoNet().to(device)  # Moves model to GPU/CPU
        if os.path.exists('emonet_pytorch_weights.pt'):
...
def list_movie_splits(args):
	...
	if args.modality == 'language':
		...
		movie_dir = os.path.join(args.project_dir, 'algonauts_2025.competitors', 'stimuli', 'movies', args.movie_type,
			args.stimulus_type)
		file_type = 'mkv'
...
def extract_visual_features(args, movie_split, feature_extractor, model_layer,
	transform, device, save_data_dir):
    ...
    save_data_dir : str
        Master path to keep the saved features.
    """
    ### Temporary directory ###
    temp_dir = os.path.join(save_data_dir, 'temp')
    ...
    for start in start_times:
        ...
        if args.model_name == 'emonet':
            ...
            nputs = torch.from_numpy(frames_array).float()
    ...
    save_dir = os.path.join(save_data_dir+'/stimulus_features/raw/',args.modality, args.model_name, args.movie_type, args.stimulus_type)
    os.makedirs(save_dir, exist_ok=True)
    out_file = os.path.join(save_dir, args.movie_type+'_'+movie_split+
        '.h5')
