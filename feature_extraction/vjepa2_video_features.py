...
def extract_fn(video: torch.Tensor, audio: torch.Tensor, transcript: List[str], verbose: bool, extractor: HuggingFaceFeatureExtractor, processor: AutoVideoProcessor, model: AutoModel, num_frames_to_extract: int) -> Dict[str, torch.Tensor]:
    with torch.no_grad():
        if video.shape[0] < 1:
            return {layer: torch.zeros_like(features_sample[layer]) for layer in extractor.layers}
