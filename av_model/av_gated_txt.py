...
class AlgonautsDataset(Dataset):
    ...
    def _load_friends_features(self, movie: str, features: Dict):
        ...
        for episode_file in episode_files:
            ...
            #Language features
            lang_file = self.features_dir['language'] / 'friends' / 'meta-llama__Llama-3.2-1B' / 'context-short_window-32.h5'
    ...
    def _load_movie10_features(self, movie: str, features: Dict):
        ...
        for partition in partitions:
            ...
            #Language features
            lang_file = self.features_dir['language'] / 'movie10' / 'meta-llama__Llama-3.2-1B' / 'context-short_window-32.h5'
            with h5py.File(lang_file, 'r') as f:
...
class TextConditioning(nn.Module):
    ...
    """
    def __init__(self, latent_dim: int, num_heads: int, enc_dropout: float, dropout: float):
        super().__init__()
    ...
    def forward(self, fused_stream: torch.Tensor, text_stream: torch.Tensor) -> torch.Tensor:
        ...
        cond_stream = self.norm(text_context + fused_stream)
        ff_out = self.txt_ffn(cond_stream)
...
class AudioVisualLangfMRIModel(L.LightningModule):
    ...
    def __init__(self, config: Dict):
        ...
        self.text_cond = TextConditioning(
            latent_dim=self.latent_dim,
            num_heads=self.num_attn_heads,
            enc_dropout=self.encoder_dropout_prob,
    ...
    def forward(
        ...
        audio_latent = self.audio_proj(audio.squeeze())  # (B, T, D)
        ...
        # Temporal processing
        temporal_features = self.temporal_processor(txt_feat)  # (B, D)

        # txt_feat = self.text_cond(temporal_features, lang_latent).squeeze()
        
        # Final projection to fMRI space
        fmri_reconstruction = self.fmri_proj(temporal_features)  # (B, 1000)
...
if __name__ == '__main__':
    ...
    for i, batch in enumerate(val_loader):
        vision, audio, language, fmri = batch['video'], batch['audio'], batch['language'], batch['fmri']
    ...
    exp_name = f"avnorm_textMHA32_sub{subject}_hrf{hrf_delay}_sw{stimulus_window}_{timestamp}"
