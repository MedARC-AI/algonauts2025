...
uv=/home/connor/.local/bin/uv

# Model 1
"$uv" run python train_feature_encoder.py \
    --overrides \
    out_dir=output/feature_encoding_enc_kernel-45_${now} \
    model.encoder_kernel_size=45\
    datasets_root=/home/connor/algonauts2025.clean/datasets/ #/dev/shm/algonauts2025
...
python3 ensemble_predictions.py --target_filename "fmri_predictions_friends_s7.npy" --foldernames $foldernames
 ...
 #Issues: 
 # 1. Connor set the data path to /dev/shm/algonauts2025 which I could not find, because of the way how llama features were coded differently than others, I have to point to Connor's home directory for those data features instead of the /data/algonauts_2025.features, can we make a symbolic link? 
 # 2. I edited my version of ./config/default_feature_encoding.yaml so that we have more features available but still some are not available in Connor's home directory
